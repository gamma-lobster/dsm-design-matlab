...
%
% Current timing support covers DAC pulses fully contained in one sample
% period, i.e. 0 <= tdac(:,1) < tdac(:,2) <= 1.
...
function [model_name, model_path, info] = build_ct_dsm_simulink_model(varargin)
    ...
    unsupported = tdac2(2:end, 1) < 0 | tdac2(2:end, 2) > 1 | ...
        tdac2(2:end, 2) <= tdac2(2:end, 1);
    if any(unsupported)
        error('build_ct_dsm_simulink_model:UnsupportedTdac', ...
            ['This builder currently supports only DAC pulses fully contained ' ...
             'within one sample period: 0 <= t1 < t2 <= 1.']);
    end
    ...
    for k = 1:n_feedback
        ...
        if abs(t1) < 1e-12 && abs(t2 - 1) < 1e-12
            add_block('simulink/Signal Routing/Goto', [model sprintf('/%s', branch_tag)], ...
                ...
                'GotoTag', branch_tag, ...
                'TagVisibility', 'global');
            add_line(model, sprintf('from_vq_%d/1', k), sprintf('%s/1', branch_tag));
        else
            pulse_width_pct = 100 * (t2 - t1);
            ...
            add_block('simulink/Signal Routing/Goto', [model sprintf('/%s', branch_tag)], ...
                ...
                'GotoTag', branch_tag, ...
                'TagVisibility', 'global');
            add_line(model, sprintf('from_vq_%d/1', k), sprintf('dac_shape_%d/1', k), 'autorouting', 'on');
